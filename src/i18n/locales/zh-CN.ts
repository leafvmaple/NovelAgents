export const zhCN = {
  agent: {
    providerRetry: "{purpose} 遇到 Provider 瞬时失败，正在进行第 {attempt} 次重试……",
  ...
  cli: {
    specHeading: "小说规格",
    title: "书名：{value}", genre: "类型：{value}", tone: "基调：{value}", pov: "视角：{value}",
    chapters: "章节：{count} 章，每章约 {words} 字", logline: "一句话：{value}", planHeading: "章节计划",
    mockNotice: "未检测到 OPENROUTER_API_KEY，当前使用离线演示 Provider。复制 .env.example 为 .env 并填写 Key 可调用真实免费模型。",
    resumedFrom: "从状态继续：{path}", completeHeading: "生成完成", chapterCount: "章节数：{count}",
    novelPath: "小说：{path}", statePath: "状态：{path}", tracePath: "调用轨迹：{path}",
  ...
  errors: {
    ...
    BLUEPRINT_POV_CHARACTER_MISSING: "章节引用了不存在的视角角色：{characterId}。",
