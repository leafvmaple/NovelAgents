...
test("message catalogs interpolate typed keys", () => {
  assert.equal(
    translate("zh-CN", "agent.recordingMemory", { chapter: 3 }),
    "连续性 Agent 正在记录第 3 章状态……",
  );
  assert.equal(
    translate("en-US", "agent.recordingMemory", { chapter: 3 }),
    "The continuity agent is recording chapter 3…",
  );
});
...
test("structured errors keep machine codes separate from localized messages", () => {
  ...
  assert.equal(
    ...
    "Chapter 2 did not pass final review.",
  );
});
